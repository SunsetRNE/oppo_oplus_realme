...
# ===== 设置自定义参数 =====
echo "===== 欧加真SM8850通用6.12.23 A16 OKI内核本地编译脚本 By Coolapk@SunsetRNE ====="
echo ">>> 读取用户配置..."
...
echo "正在克隆源码仓库..."
aria2c -s16 -x16 -k1M https://github.com/SunsetRNE/android_kernel_common_oneplus_sm8850/archive/refs/heads/oneplus/sm8850_v_16.0.0_oneplus_15.zip -o common.zip && 
unzip -q common.zip && 
...
mkdir -p clang19 &&
aria2c -s16 -x16 -k1M https://github.com/SunsetRNE/oneplus_sm8650_toolchain/releases/download/LLVM-Clang19-r536225/clang-r536225.zip -o clang.zip &&
unzip -q clang.zip -d clang19 &&
rm -rf clang.zip &

echo "正在克隆Rust 1.82.0工具链..." &&
mkdir -p rust &&
aria2c -s16 -x16 -k1M https://github.com/SunsetRNE/oneplus_sm8650_toolchain/releases/download/LLVM-Clang19-r536225/rust.zip -o rust.zip &&
unzip -q rust.zip -d rust &&
rm -rf rust.zip &

echo "正在克隆构建工具..." &&
aria2c -s16 -x16 -k1M https://github.com/SunsetRNE/oneplus_sm8650_toolchain/releases/download/LLVM-Clang19-r536225/build-tools.zip -o build-tools.zip &&
...
if [[ $KSU_BRANCH == [yYrR] ]]; then
  ...
  echo 'CONFIG_KSU_FULL_NAME_FORMAT="%TAG_NAME%-%COMMIT_SHA%@SunsetRNE"' >> ./common/arch/arm64/configs/gki_defconfig
elif [[ "$KSU_BRANCH" == "n" || "$KSU_BRANCH" == "N" ]]; then
  ...
  wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/other_patch/apk_sign.patch
...
if [[ "$APPLY_SUSFS" == [yY] ]]; then
  echo ">>> 克隆补丁仓库..."
  git clone --depth=1 https://github.com/SunsetRNE/susfs4oki.git susfs4ksu -b oki-android16-6.12
...
if [[ "$APPLY_LZ4" == "y" || "$APPLY_LZ4" == "Y" ]]; then
  ...
  git clone --depth=1 https://github.com/SunsetRNE/oppo_oplus_realme_sm8850.git
...
if [[ "$APPLY_LZ4KD" == "y" || "$APPLY_LZ4KD" == "Y" ]]; then
  echo ">>> 应用 LZ4KD 补丁..."
  cd "$WORKDIR/kernel_workspace/common"
  wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/other_patch/lz4kd.patch
...
cd common
wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/other_patch/cve-2026-43499-rtmutex-6.12.patch
...
if [[ "$APPLY_BETTERNET" == "y" || "$APPLY_BETTERNET" == "Y" ]]; then
  ...
  cd common
  wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/other_patch/config.patch
...
if [[ "$APPLY_DROIDSPACES" == [sSeE] ]]; then
  ...
  wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/droidspaces_patch/fix_sysvipc_kabi_a16-6.12.patch
  patch -p1 -F 3 < fix_sysvipc_kabi_a16-6.12.patch || true
  # 修补 oplus_bsp_midas 行为，避免开机崩溃
  wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/droidspaces_patch/fix_oplus_bsp_midas.patch
  patch -p1 -F 3 < fix_oplus_bsp_midas.patch || true
  # 应用 NTSync 补丁
  wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/droidspaces_patch/ntsync_compat_android16-6.12.patch
  ...
  if [[ "$APPLY_DROIDSPACES" == [eE] ]]; then
    ...
    # cd common
    # wget https://github.com/SunsetRNE/oppo_oplus_realme_sm8850/raw/refs/heads/main/droidspaces_patch/evdi_drm.patch
...
if [[ "$APPLY_LZ4KD" == "y" || "$APPLY_LZ4KD" == "Y" ]]; then
  wget https://raw.githubusercontent.com/SunsetRNE/oppo_oplus_realme_sm8850/refs/heads/main/zram.zip
fi
